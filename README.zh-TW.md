...
- `upload_file` 預設僅允許存取 inbox 目錄內的檔案，防止 prompt injection 竊取任意檔案。在 `access.json` 設定 `fullAccess: true` 可允許任意路徑，請僅在完全信任所有 `allowFrom` 用戶時啟用
- 檔案上傳密碼使用 `crypto.randomBytes` 生成（96-bit 熵），以 SHA-256 雜湊後儲存，符合 gofile.io 的驗證機制
